...
var (
	...
	defaultACMEDNS01SolverNameservers = ""
)
...
func (s *ControllerOptions) AddFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&s.ACMEDNS01SolverNameservers, "acme-dns01-solver-nameservers", defaultACMEDNS01SolverNameservers, ""+
		"A comma separated list of nameserver:port to use when solving the ACME DNS01 challenge. "+
		"This can be used to override the nameservers found in the pod's resolv.conf file if "+
		"you are running Kubernetes outside of the public cloud and want challenge requests to "+
		"go to AWS or Google DNS providers. Default is an empty string which will use the pod's "+
